...
from cell_features import get_trj_idx
...
def create_id_masks(merged_masks, trj, background_id):
    id_masks = np.zeros(merged_masks.shape, dtype=np.float16)  # Fill with background
    for i_frame in range(merged_masks.shape[2]):
        new_frame = np.zeros(merged_masks.shape[:2], dtype=np.float16)
